...
import static min3d.vos.PageFlip.*;

public class DLauncherAPP extends RendererActivity {
	...
	private static final int mPlanesNumber = 5;
	...
	private final Runnable zoomInRunnable = new Runnable() {
		public void run() {
			...
			//int faceindex = sceneAngle > 0 ? getRelativeIndex((int) ((-sceneAngle - 5) / pageDistanceMeasure),
				//	routes[Flip_Method.ordinal()].length) : getRelativeIndex(
					//(int) ((-sceneAngle + 5) / pageDistanceMeasure), routes[Flip_Method.ordinal()].length);
	...
	public void updateScene() {
		// Log.d("hydra" , ""+bTouched+getRelativeFloat(sceneAngle,pageDistanceMeasure)) ;
	...
	private void initBackGround() {
		float aspectRatio = mTouchBorder.aspectRatio;
		float fixedsize = aspectRatio > 1 ? 5f : 3f;
		_background = new Rectangle(fixedsize * 4, fixedsize * 4 / aspectRatio, 20, 40, new Color4());
		...
		_background.textures().addById("backover");
		_background.rotation().y = 180;
		_background.position().z = -10f;
		_background.lightingEnabled(true);
	...
	private void initTextures() {
		...
		b = Utils.makeBitmapFromResourceId(this, R.drawable.desert);
	...
	private void loadApps() {
		...
		lmApps = getPackageManager().queryIntentActivities(mainIntent, 0);
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menu.add(0, Menu.FIRST + 0, 0, "EFFECTS").setIcon(android.R.drawable.ic_menu_gallery)
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		/*
		 * Flip_Method =
		 * PageFlip.FLIP_TYPE.class.getEnumConstants()[item.getItemId() - 1];
		 * initVariables(); initLauncherTextures(); dynamicChangeTexture();
		 * return true;
		 * 
		...
		case Menu.FIRST + 0:
			Intent intent = new Intent();
			intent.setClass(this, SpinnerEffects.class);
			bInitReady = false;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 0 && resultCode == Activity.RESULT_OK) {
			...
			Flip_Method = FLIP_TYPE.class.getEnumConstants()[bundle.getInt("Effect")];
			initVariables();
			initLauncherTextures();
			dynamicChangeTexture();
	...
	protected void onCreateSetContentView() {
		...
		addContentView(textBox, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
