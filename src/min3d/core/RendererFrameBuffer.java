...
public class RendererFrameBuffer implements GLSurfaceView.Renderer {
	...
	public RendererFrameBuffer(Scene $scene , Scene $scene1) {
		_scene = $scene;
		_scene1 = $scene1;
	...
	public void onSurfaceCreated(GL10 $gl, EGLConfig eglConfig) {
        ...
        Shared.textureManager().addTextureId(mFramebuffer, "MyFrameBuffer") ;
    ...
    private int createTargetTexture( int width, int height) {
        ...
        texture = textures[0];
        _gl.glBindTexture(GL10.GL_TEXTURE_2D, texture);
        _gl.glTexImage2D(GL10.GL_TEXTURE_2D, 0, GL10.GL_RGBA, width, height, 0,
                GL10.GL_RGBA, GL10.GL_UNSIGNED_BYTE, null);
        return texture;
    }
	
    private int createFrameBuffer( int width, int height, int targetTextureId) {
        GL11ExtensionPack gl11ep = (GL11ExtensionPack) _gl;
        int framebuffer;
	...
	protected void drawScene1() {
		...
		for (int i = 0; i < _scene1.children().size(); i++) {
			Object3d o = _scene1.children().get(i);
			/*
			 * if(o.animationEnabled()) { ((AnimationObject3d)o).update(); }
			 */
			drawObject(o);
...
/*		_gl.glEnable(GL10.GL_DEPTH_TEST);   
		...
		_gl.glStencilFunc(GL10.GL_EQUAL, 1, 1); 
		_gl.glStencilOp(GL10.GL_KEEP, GL10.GL_KEEP, GL10.GL_KEEP);*/
		// Texture
		_gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MIN_FILTER, GL10.GL_NEAREST); // (OpenGL
																								// default
																								// is
																								// GL_NEAREST_MIPMAP)
