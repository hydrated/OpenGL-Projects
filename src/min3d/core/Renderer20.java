...
public class Renderer20 implements GLSurfaceView.Renderer
{
	...
	*//**
	...
	protected void drawSetup()
	{
		...
		// Camera 
			...
			_scene.camera().setDirtyFlag(false) ;
		}
		// Background color
		...
		//if(_scene.lightingEnabled())drawSetupLights();
	...
	protected void drawSetupLights()
	{
		...
		for (int glIndex = 0; glIndex < NUM_GLLIGHTS; glIndex++)
		{
			if (_scene.lights().glIndexEnabledDirty()[glIndex] == true)
			{
				...
				{
					GLES20.glEnable(GL10.GL_LIGHT0 + glIndex);
				...
				{
					GLES20.glDisable(GL10.GL_LIGHT0 + glIndex);
				}
		...
		for (int i = 0; i < lights.length; i++)
		{
			...
			if (light.isDirty()) // .. something has changed
			{
				...
				{
					if (light.isVisible()) {
						GLES20.glEnable(glLightId);
					} else {
						GLES20.glDisable(glLightId);
					}
	...
	protected void drawObject(Object3d $o)
	{
		...
		{
			GLES20.glDrawArrays($o.renderType().glValue(), 0, $o.vertices().size());
		}
		...
		for (int i = 0; i < RenderCaps.maxTextureUnits(); i++)
		{
			...
			if ($o.hasUvs() && $o.texturesEnabled())
			{
				...
				if (textureVo != null)
				{
					...
					GLES20.glBindTexture(GL10.GL_TEXTURE_2D, glId);
					...
					GLES20.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MIN_FILTER, minFilterType);
					GLES20.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MAG_FILTER, GL10.GL_LINEAR); // (OpenGL default)
					...
					//_gl.glTexParameteri(GL11.GL_TEXTURE_CUBE_MAP, GL10.GL_TEXTURE_WRAP_S, GL10.GL_CLAMP_TO_EDGE);
					//_gl.glTexParameteri(GL11.GL_TEXTURE_CUBE_MAP, GL10.GL_TEXTURE_WRAP_T, GL10.GL_CLAMP_TO_EDGE);
	...
	*//**
	...
	private void reset()
	{
		...
		// Texture
		GLES20.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MIN_FILTER, GL10.GL_NEAREST); // (OpenGL default is GL_NEAREST_MIPMAP)
		GLES20.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MAG_FILTER, GL10.GL_LINEAR); // (is OpenGL default)
