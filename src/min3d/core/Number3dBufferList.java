...
public class Number3dBufferList
{
	...
	public void setOffSet(int $index , float $offsetx , float $offsety , float $offsetz)
	{			
		_b.position($index * PROPERTIES_PER_ELEMENT);
		_b.put(_b.get($index * PROPERTIES_PER_ELEMENT)+$offsetx);
		_b.put(_b.get($index * PROPERTIES_PER_ELEMENT+1)+$offsety);
		_b.put(_b.get($index * PROPERTIES_PER_ELEMENT+2)+$offsetz);
	}
