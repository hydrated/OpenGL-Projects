...
public class Renderer10 implements GLSurfaceView.Renderer {
	...
	protected void drawSetup() {
		...
		if (_scene.camera().isDirty()) {
			// Camera
			_gl.glMatrixMode(GL10.GL_MODELVIEW);
			_gl.glLoadIdentity();

			GLU.gluLookAt(_gl, _scene.camera().position.x, _scene.camera().position.y, _scene.camera().position.z,
			...
			_scene.camera().clearDirtyFlag();
		}
		// Background color

		if (_scene.backgroundColor().isDirty()) {
			_gl.glClearColor((float) _scene.backgroundColor().r() / 255f, (float) _scene.backgroundColor().g() / 255f,
					(float) _scene.backgroundColor().b() / 255f, (float) _scene.backgroundColor().a() / 255f);
	...
	protected void drawScene() {
		/*
		 * if(_scene.fogEnabled() == true) { _gl.glFogf(GL10.GL_FOG_MODE,
		 * _scene.fogType().glValue()); _gl.glFogf(GL10.GL_FOG_START,
		 * _scene.fogNear()); _gl.glFogf(GL10.GL_FOG_END, _scene.fogFar());
		 * _gl.glFogfv(GL10.GL_FOG_COLOR, _scene.fogColor().toFloatBuffer() );
		 * _gl.glEnable(GL10.GL_FOG); } else { _gl.glDisable(GL10.GL_FOG); }
		 */

		for (int i = 0; i < _scene.children().size(); i++) {
			Object3d o = _scene.children().get(i);
			/*
			 * if(o.animationEnabled()) { ((AnimationObject3d)o).update(); }
			 */
			drawObject(o);
		}
	}
	...
	protected void drawObject(Object3d $o) {
		if ($o.isVisible() == true) {
			 ...
			 * _gl.glGetIntegerv(GL10.GL_LIGHTING, _scratchIntBuffer); if
			 * (useLighting != _scratchIntBuffer.get(0)) { if (useLighting == 1)
			 * { _gl.glEnable(GL10.GL_LIGHTING); } else {
			 * _gl.glDisable(GL10.GL_LIGHTING); } }
			 */

			boolean useLighting = (_scene.lightingEnabled() && $o.hasNormals() && $o.normalsEnabled() && $o
					.lightingEnabled());
			if (useLighting) {
			...
			/*
			 * boolean useBlend = ($o.hasBlend()); // boolean useBlend = false ;
			 * // _gl.glDisable(GL10.GL_DEPTH_TEST); if (useBlend) {
			 * _gl.glEnable(GL10.GL_BLEND); _gl.glBlendFunc(GL10.GL_SRC_ALPHA,
			 * GL10.GL_ONE_MINUS_SRC_ALPHA); _gl.glDisable(GL10.GL_DEPTH_TEST);
			 * } else { _gl.glDisable(GL10.GL_BLEND); }
			 */

			// Shademodel

			/*
			 * _gl.glGetIntegerv(GL11.GL_SHADE_MODEL, _scratchIntBuffer); if
			 * ($o.shadeModel().glConstant() != _scratchIntBuffer.get(0)) {
			...
			if ($o.hasVertexColors() && $o.vertexColorsEnabled()) {
				...
				_gl.glColor4f((float) $o.defaultColor().r / 255f, (float) $o.defaultColor().g / 255f,
						(float) $o.defaultColor().b / 255f, (float) $o.defaultColor().a / 255f);
			 ...
			 * _scratchB = (_scratchIntBuffer.get(0) != 0); if
			 * ($o.colorMaterialEnabled() != _scratchB) { if
			 * ($o.colorMaterialEnabled()) _gl.glEnable(GL10.GL_COLOR_MATERIAL);
			 ...
			 * // Point size
			 * 
			 * if ($o.renderType() == RenderType.POINTS) { if
			 * ($o.pointSmoothing()) _gl.glEnable(GL10.GL_POINT_SMOOTH); else
			 ...
			 * // Line properties
			 * 
			 * if ($o.renderType() == RenderType.LINES || $o.renderType() ==
			 * RenderType.LINE_STRIP || $o.renderType() == RenderType.LINE_LOOP)
			 * { if ( $o.lineSmoothing() == true) {
			 * _gl.glEnable(GL10.GL_LINE_SMOOTH); } else {
			...
			if ($o.getReverse()) {
				//Log.d("hydra" ,"reversing") ;
				_gl.glTranslatef(-$o.position().x - $o.position_offset().x, $o.position().y + $o.position_offset().y,
						-$o.position().z - $o.position_offset().z);
			} else {
				_gl.glTranslatef($o.position().x + $o.position_offset().x, $o.position().y + $o.position_offset().y,
						$o.position().z + $o.position_offset().z);
			}
			...
			if (!$o.ignoreFaces()) {
				...
				_gl.glDrawElements($o.renderType().glValue(), len * FacesBufferedList.PROPERTIES_PER_ELEMENT,
			...
			_gl.glScalef($o.scale().x, $o.scale().y, $o.scale().z);
			if ($o instanceof Object3dContainer && $o.isDrawChildren() == true) {
				Object3dContainer container = (Object3dContainer) $o;
	...
	private void simpleDrawObject_textures(Object3d $o) {
		...
		if ($o.hasUvs() && $o.texturesEnabled()) {
			...
			TextureVo textureVo = ((0 < $o.textures().size())) ? textureVo = $o.textures().get(0) : null;
			...
			if (textureVo != null) {
				...
				int minFilterType = _textureManager.hasMipMap(textureVo.textureId) ? GL10.GL_LINEAR_MIPMAP_NEAREST
						: GL10.GL_LINEAR;
				...
				_gl.glTexParameterx(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_S, (textureVo.repeatU ? GL10.GL_REPEAT
						: GL10.GL_CLAMP_TO_EDGE));
				_gl.glTexParameterx(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_T, (textureVo.repeatV ? GL10.GL_REPEAT
						: GL10.GL_CLAMP_TO_EDGE));
			...
			if ($o.hasUvs() && $o.texturesEnabled()) {
				...
				if (textureVo != null) {
					...
					int minFilterType = _textureManager.hasMipMap(textureVo.textureId) ? GL10.GL_LINEAR_MIPMAP_NEAREST
							: GL10.GL_LINEAR;
					_gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MIN_FILTER, minFilterType);
					_gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MAG_FILTER, GL10.GL_LINEAR); // (OpenGL
																											// default)

					// do texture environment settings
					for (int j = 0; j < textureVo.textureEnvs.size(); j++) {
						_gl.glTexEnvx(GL10.GL_TEXTURE_ENV, textureVo.textureEnvs.get(j).pname,
					...
					_gl.glTexParameterx(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_T, (textureVo.repeatV ? GL10.GL_REPEAT
							: GL10.GL_CLAMP_TO_EDGE));
					// _gl.glTexParameteri(GL11.GL_TEXTURE_CUBE_MAP,
	...
	int uploadTextureAndReturnId(Bitmap $bitmap, boolean $generateMipMap) /*
																		 * package-
																		 * private
																		 */
	{
	...
	protected void updateViewOrtho() {
		FrustumManaged vf = _scene.camera().frustum;
		float n = vf.shortSideLength() / 2f;
		...
		_gl.glMatrixMode(GL10.GL_PROJECTION);
		_gl.glLoadIdentity();
		_gl.glOrthof(lt * 5, rt * 5, btm * 5, top * 5, vf.zNear(), vf.zFar());
	...
	private final int fpsFrameCount = 120;
	...
	private void doFps() {
		...
		if (delta >= FRAMERATE_SAMPLEINTERVAL_MS) {
			...
			_activityManager.getMemoryInfo(_memoryInfo);
			Log.v(Min3d.TAG, "FPS: " + Math.round(_fps) + ", availMem: " + Math.round(_memoryInfo.availMem / 1048576)
					+ "MB");
	...
	private void reset() {
		...
		_gl.glDepthFunc(GL10.GL_LEQUAL);
		 ...
		 * _gl.glStencilFunc(GL10.GL_EQUAL, 1, 1); _gl.glStencilOp(GL10.GL_KEEP,
		 * GL10.GL_KEEP, GL10.GL_KEEP);
		 */
		// Texture

		_gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MIN_FILTER, GL10.GL_LINEAR); // (OpenGL
																								// default
																								// is
																								// GL_NEAREST_MIPMAP)
		_gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MAG_FILTER, GL10.GL_LINEAR); // (is
